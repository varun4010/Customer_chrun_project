...
export default function DashboardPage() {
  ...
  const metrics = [
    ...
    { label: 'F1 (Tuned Thresh)', value: metadata.test_f1_tuned?.toFixed(4), color: 'emerald', icon: '✅', desc: 'F1 with optimized threshold' },
    { label: 'F1 (Default 0.5)', value: metadata.test_f1_default?.toFixed(4), color: 'amber', icon: '📊', desc: 'F1 with default 0.5 threshold' },
    { label: 'CV F1 (Best)', value: metadata.best_cv_f1?.toFixed(4), color: 'cyan', icon: '🔁', desc: '5-Fold cross-validation F1' },
    { label: 'Tuned Threshold', value: `${(metadata.best_threshold * 100).toFixed(1)}%`, color: 'violet', icon: '⚡', desc: 'Optimal decision threshold from validation set' },
    ...
    <div className="fade-in">
      ...
      <div className="grid-2" style={{ gap: 24 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          {/* Threshold Analysis */}
          <div className="card">
            <div className="section-title">⚡ Threshold Tuning Impact</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {[
                { label: 'Default Threshold (0.5)', f1: metadata.test_f1_default, color: 'var(--amber)' },
                { label: `Tuned Threshold (${(metadata.best_threshold*100).toFixed(1)}%)`, f1: metadata.test_f1_tuned, color: 'var(--emerald)' },
              ].map(t => (
                <div key={t.label}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                    <span style={{ fontSize: 13, color: 'var(--text-secondary)' }}>{t.label}</span>
                    <span style={{ fontSize: 13, fontWeight: 700, color: t.color, fontFamily: 'JetBrains Mono' }}>{t.f1?.toFixed(4)}</span>
                  </div>
                  <div className="progress-bar">
                    <div className="progress-fill" style={{ width: `${t.f1 * 100}%`, background: t.color }} />
                  </div>
                </div>
              ))}
              <div style={{ padding: '10px 12px', marginTop: 4, background: 'rgba(16,185,129,0.08)', border: '1px solid rgba(16,185,129,0.2)', borderRadius: 10, fontSize: 12, color: 'var(--emerald)' }}>
                ✅ Threshold tuning improved F1 by <strong>+{((metadata.test_f1_tuned - metadata.test_f1_default) * 100).toFixed(2)}%</strong> absolute
              </div>
            </div>
          </div>
