...
def train_and_export():
    ...
    print("=== Step 7: Threshold Tuning on Validation Set ===")
    ...
    ypr_tuned = (test_proba >= best_threshold).astype(int)
    ypr_default = (test_proba >= 0.5).astype(int)

    test_prauc = float(average_precision_score(yts, test_proba))
    test_f1_tuned = float(f1_score(yts, ypr_tuned))
    test_f1_default = float(f1_score(yts, ypr_default))

    print(f"Final Test PR-AUC: {test_prauc:.4f}")
    print(f"Final Test F1 (tuned threshold): {test_f1_tuned:.4f} | Default (0.5): {test_f1_default:.4f}")
    print(classification_report(yts, ypr_tuned, target_names=["Stayed (0)", "Churned (1)"]))

    print("=== Step 9: Extract Feature Importances ===")
    model_obj = final_pipeline.named_steps['xgb']
